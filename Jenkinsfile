pipeline {
    ...
    environment{
        ...
        // 项目名
        //DOCKER_PROJECT_NAME = 's2c-shop-api'
    }
    options {
        //保持构建的最大个数
            ...
            steps{
                ...
                script{
                    ...
                    if (env.TAG_NAME){
                        ...
                        env.onlineImage = "${env.DOCKER_REPO}/${env.CHEDIAN_DOCKER_TX_NAMESPACE_BACKEND}/${env.dockerPreImage}:${env.TAG_NAME}"
                    } else {
                        // 镜像名
                        env.dockerPreImage = env.DOCKER_PROJECT_NAME + '-' + env.BRANCH_NAME.split("/").join("-")
                        env.dockerImage = env.dockerPreImage + ':' + env.GIT_COMMIT
                        env.onlineImage = "${env.DOCKER_REPO}/${env.CHEDIAN_DOCKER_TX_NAMESPACE_BACKEND}/${env.dockerPreImage}:${env.GIT_COMMIT}"
                    }
                }
                echo "构建准备结束。。。"
            }
            ...
            steps{
                ...
                script{
                    ...
                    if (env.TAG_NAME) {
                        ...
                        env.k8sNamespace = 's2c-prod-backend';
                    } else if (env.BRANCH_NAME == 'build/dev') {
                        env.k8sNamespace = 's2c-dev-backend';
                        env.needDeploy = "Yes"
                    } else if (env.BRANCH_NAME == 'build/test') {
                        env.k8sNamespace = 's2c-test-backend';
                        env.needDeploy = "Yes"
                    } else if (env.BRANCH_NAME == 'build/docs') {
                        env.k8sNamespace = 's2c-docs';
                        env.needDeploy = "Yes"
                    } else {
                        false
                    }
                    ...
                    if ("${env.needDeploy}".contains("Yes")) {
                        ...
                        withCredentials([kubeconfigContent(credentialsId: env.k8sConfigId, variable: 'KUBECONFIG_CONTENT')]) {
                            sh '''
                                ...
                                whoami
                                kubectl set image deployment/shop-api shop-api=${onlineImage} --namespace=${k8sNamespace} --kubeconfig=kubeconfig  --insecure-skip-tls-verify=true
                                rm kubeconfig
                            '''
                        }
                        echo "部署服务器完成。。。"
